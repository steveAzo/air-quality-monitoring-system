...
def get_latest_weather(lat: float = 5.58389, lon: float = -0.19968, db: Session = Depends(get_db)):
    """
    Fetch latest MERRA-2 weather and aerosol data for a given location and save to database.
    Returns temperature (T2M), wind speed (U10M), wind direction (V10M), humidity (QV2M),
    ozone (TO3), and PM2.5 (calculated from aerosol components).
    """
    try:
        ...
        # Convert timestamp
        timestamp = datetime.utcfromtimestamp(point_data["time"].values[-1].astype('O') / 1e9)
        ...
        # Save to database
        db_weather = weather_crud.create_weather_data(db, weather_data)
        logger.info(f"Successfully saved weather data to database with ID: {db_weather.id}")
        ...
        return [
            {
                ...
                "lat": lat,
                "lon": lon,
                "temperature": data.get("T2M", None),
                "wind_speed": data.get("U10M", None),
                "wind_direction": data.get("V10M", None),
                "humidity": data.get("QV2M", None),
                "ozone": data.get("TO3", None),
                "pm25": pm25
            }
        ]

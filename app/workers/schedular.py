...
def get_sensor_parameter_name(sensor_id: int, db) -> str:
    ...
    try:
        ...
        if sensor and sensor.parameter_name:
            ...
            logger.info("    ⚠️  Sensor not found in DB or has no parameter name")
        ...
        if parameter_name:
            ...
            logger.warning("    ⚠️  No parameter name found in API response for sensor %s", sensor_id)
...
def refresh_latest_for_location(location_id: int):
    ...
    try:
        ...
        if not latest_results:
            logger.warning("  ⚠️  No results returned from API for location %s", location_id)
            return
        ...
        for i, measurement_data in enumerate(latest_results):
            try:
                ...
                if not sensor_id:
                    logger.warning("    ⚠️  No sensor_id found in measurement data, skipping")
                    continue
...
def start_scheduler(interval_minutes: int = 50):
